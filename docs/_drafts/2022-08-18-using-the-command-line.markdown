...
| `man *command*` | manual | It displays a full manual on how to use the chosen command | `man ls` would display <p><img src="/assets/images/commandlinemanual.png" alt="Command line manual example" width="300"></p> |
...
|`head -number *file*` | Head | It will output the specified number of lines. If the number is empty, it will output 10 lines of text | `head -3 MyFile` or `head MyFile`|
|`tail *file*`
...
Flags are a combination of a hyphon and a letter that we can use to change the functionality of the commands. You can use several flags on a single command, and instead of writing `ls -l -A` you can also combine them: `ls -lA`
